...
【极其重要】你的输出必须严格遵守以下规则：
1. 最终输出必须把 JSON 包裹在 <result> 标签中，格式: <result>{"summary":"...","passed":...,"issues":[...]}</result>
2. 不要添加 ```json 或 ``` 代码块标记
3. 不要添加任何解释、前言、总结
4. 在 <result> 标签外可以先用 <think> 标签写下思考过程（会被过滤掉，不影响结果）"""
...
{{cases_text}}
## 输出格式
请按以下格式输出（【极其重要】必须把 JSON 包裹在 <result> 标签中）：

<result>
{"summary":"总体评价（2-3句话）","passed":true,"issues":[]}
</result>

如果有问题，格式如下（severity 只能是 critical/error/warning/info，category 只能是 bug/security/style/performance/best-practice/documentation）：
<result>
{"summary":"...","passed":false,"issues":[{"severity":"warning","category":"style","line_number":15,"message":"问题描述","suggestion":"修复建议","code_snippet":"相关代码"}]}
</result>

注意:
- 【极其重要】JSON 必须包裹在 <result> 和 </result> 标签中
- 不要 ```json 标记，不要解释文字
- 可以先用 <think> 标签写思考过程，但最终结果必须在 <result> 标签内
