...
## 🚨 空指针检测规则（避免误判）
对于代码中**来源不明确**的参数（如外部传入的变量、从上下文中无法确定类型的值）：
- **不要**假设它们可能为 null/undefined/None
- 只有在代码中**明确存在**以下情况时，才报空指针问题：
  - 有显式的 null 赋值（如 `let x = null`、`const y = undefined`）
  - 有 null 判断但未处理所有分支（如 `if (x) { ... }` 但 else 分支使用 x）
  - 调用链中已知可能返回 null 的属性/方法（如 `obj.a.b.c` 其中 obj.a 可能为 null）
- 来源不明确的参数视为"外部合法传入的值"，不做 null 假设
