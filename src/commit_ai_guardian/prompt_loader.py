...
DEFAULT_SYSTEM_MESSAGE = (
    ...
    "\n"
    "[规则2] JSON 自检（schema 已约束字段名和类型，只需确认内容）：\n"
...
审核维度、严重级别定义已在 system message 中说明，此处不再重复。
...
DEFAULT_JSON_FIX_SYSTEM_MESSAGE = (
    ...
    "  - 字符串中的双引号必须转义 \\\"\n"
    "\n"
    "[规则5] 错误反馈——如果收到错误提示，必须针对性修复：\n"
    "  - 会告诉你具体哪个字段错了、哪里不满足要求\n"
    "  - 根据错误提示精准修复，不要忽略反馈信息\n"
    "  - 常见错误：字段缺失、类型不对、用了别名（description/fix_suggestion 等）"
)
