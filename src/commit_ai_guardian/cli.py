...
def configure(config_path):
    ...
    # 根据 api_base 推荐模型（与 Demo 页面保持一致）
    api_base_lower = config.api_base.lower()
    click.echo("")
    if "minimax" in api_base_lower:
        click.echo("  MiniMax 推荐模型：")
        click.echo("  +---------------------+-------------------------------+------------+")
        click.echo("  | 模型名称            | 说明                          | 上下文     |")
        click.echo("  +---------------------+-------------------------------+------------+")
        click.echo("  | MiniMax-M3          | 最新，1M 上下文（推荐）        | 1,048,576  |")
        click.echo("  | MiniMax-M2.7        | 通用场景首选                   | 200K       |")
        click.echo("  | MiniMax-M2.5        | 性价比之选                     | 200K       |")
        click.echo("  | MiniMax-M2.1        | 编程专项                       | 100K       |")
        click.echo("  +---------------------+-------------------------------+------------+")
        ...
        click.echo("  | 模型名称            | 说明                          | 上下文     |")
        click.echo("  +---------------------+-------------------------------+------------+")
        click.echo("  | deepseek-chat       | 通用对话（推荐）               | 64K        |")
        click.echo("  | deepseek-reasoner   | 推理模型，适合复杂逻辑审查     | 64K        |")
        click.echo("  +---------------------+-------------------------------+------------+")
        ...
        click.echo("  | 模型名称            | 说明                          | 上下文     |")
        click.echo("  +---------------------+-------------------------------+------------+")
        click.echo("  | kimi-k2             | 最新，超长上下文（推荐）       | 256K       |")
        click.echo("  | kimi-k2-thinking    | 推理模型，适合复杂代码分析     | 256K       |")
        click.echo("  +---------------------+-------------------------------+------------+")
        ...
        click.echo("  | 模型名称            | 说明                          | 上下文     |")
        click.echo("  +---------------------+-------------------------------+------------+")
        click.echo("  | gpt-4o              | 最强模型，适合复杂审查         | 128K       |")
        click.echo("  | gpt-4o-mini         | 性价比之选，速度快（推荐）     | 128K       |")
        click.echo("  | gpt-3.5-turbo       | 基础模型，成本低               | 16K        |")
        click.echo("  +---------------------+-------------------------------+------------+")
    else:
        click.echo("  模型名称示例：")
