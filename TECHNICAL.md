...
1. [功能概览](#1-功能概览)
2. [系统架构](#2-系统架构)
3. [核心数据流（开发者视角）](#3-核心数据流开发者视角)
4. [Prompt 工程（Prompt 工程师视角）](#4-prompt-工程prompt-工程师视角)
5. [审核规则（规则设计者视角）](#5-审核规则规则设计者视角)
6. [案例系统（案例编写者视角）](#6-案例系统案例编写者视角)
7. [缓存与日志（运维/开发者视角）](#7-缓存与日志运维开发者视角)
8. [配置系统](#8-配置系统)
9. [Git Hook 机制（DevOps 视角）](#9-git-hook-机制devops-视角)
10. [API 容错与阻断](#10-api-容错与阻断)
11. [Think 输出控制](#11-think-输出控制)
12. [设计哲学](#12-设计哲学)
...
│           └── examples/                  # 示例案例（按语言子目录）
│               ├── js/
│               │   ├── vue-default-slot-redundant.md
│               │   └── ...
│               └── ...
├── pyproject.toml
├── uv.lock
...
Prompt 模板由以下部分组成（均为模板变量，实际内容在运行时填充）：
...
> 模板文件存放在 `.ai-review/prompts/` 目录下，`install` 命令会自动创建。可直接编辑这些文件来自定义审核行为，无需改代码。
...
配置方式：`case_format: compact`（全局或项目级 config.yaml）。详见 [第 6 节](#6-案例系统案例编写者视角) 完整说明。
...
**这不是 AI 天生懂注释，而是 system prompt 里明确规定的约束**：

> "📌 注释是代码意图的重要说明（优先级高于案例）：代码中的注释是开发者写的意图说明，比代码本身更直接。判断代码是否正确时，要参考注释的说明。如果注释明确解释了某个做法的原因，要尊重注释的判断。不要无视注释，仅凭代码表面就下结论。**当注释与案例出现冲突时，以注释为准。注释的优先级高于案例。**"

**实际效果**：

```typescript
// 这里必须用 setTimeout，因为第三方库在 DOM 更新后才初始化
setTimeout(() => initThirdParty(), 100);
```

- 没有注释 → AI 可能报"不必要的延迟"（warning）
- 有注释说明原因 → AI 尊重注释判断，**不报错**

**注释与案例冲突时的处理**：

```typescript
// 案例说 "不要用 var，用 let/const"
// 但代码注释说明了原因：
// 这里必须用 var，因为需要变量提升
var config = getConfig();
```

- 案例要求用 let/const → 但注释明确解释了用 var 的原因
- **以注释为准** → 不报错（注释优先级高于案例）

**设计意图**：
- 注释不仅是给人看的，也是给 AI 看的审核指令
- 团队可以用注释来**规定审核边界**："这里我为什么这么写"
- **注释优先级高于案例**：当两者冲突时，以注释为准
- 好注释 = 好的审核结果，降低误报率
...
| 级别 | 含义 | 阻断条件 |
|------|------|---------|
| `critical` | 严重问题 | 阻断 commit |
| `error` | 错误 | 阻断 commit |
| `warning` | 警告 | 只显示，不阻断 |
| `info` | 提示 | 只显示，不阻断 |

实际阻断 commit 的只有 `error` 和 `critical` 级别的问题。`info` 和 `warning` 只会在报告中显示，不会阻止提交。
...
描述该案例要检测的问题类型和表现。简明扼要地说明"这是什么问题"。

##### 坏代码

用 `## 坏代码` 标记，包含一个或多个反例代码块。每个反例可以用 `### 场景名` 进一步细分：
...
case_format: compact  # default / compact / minimal
```
...
审核过的文件自动缓存，内容未变直接复用结果。关闭缓存只跳过缓存检查/写入，日志文件照常生成。
...
| `max_tokens` | AI 最大返回 token 数 | `8192` |
...
无 husky 时的安装位置：`.git/hooks/pre-commit`
...
# 输出 .husky/_ 或 .husky → husky 在控制，工具自动安装到 .husky/pre-commit
# 输出为空 → 原生 hook，工具安装到 .git/hooks/pre-commit
```
...
**lint-staged 和 AI 审核共存的 `.husky/pre-commit`：**
...
在调用 AI 修复之前，先尝试 7 种本地策略（成本低、速度快）：

| 策略 | 处理的问题 |
|------|-----------|
| 1. 直接解析 | 正常情况，直接成功 |
| 2. BOM 去除 | Windows 编辑器遗留的 `\uFEFF` 头 |
| 3. 单引号→双引号 | AI 用单引号写 JSON |
| 4. Trailing comma 去除 | 最后一个元素后多逗号 |
| 5. 注释去除 | `//` 或 `/* */` 注释 |
| 6. 非法转义修复 | `\x` 等非标准转义序列 |
| 7. 括号补全 | 流式输出被截断，缺失 `}` 或 `]` |

**快速通道**：空数组 `"[]"` 和空对象 `"{}"` 在 tryParseJson 之前直接拦截，避免误判为解析失败。

#### AI 修复（3 次尝试 + 错误反馈）

本地 7 种策略都失败时，调用专门的 JSON 修复 AI：

```
Attempt 1: 原始损坏 JSON → AI修 → Schema校验
Attempt 2: 原始JSON + 上次结果(assistant) + 错误反馈(user) → AI修 → Schema校验
Attempt 3: 同上，最后一次机会
```

**对话角色对齐**：
- `assistant` 角色 = AI 上次修复的输出（`last_fixed_result`）
- `user` 角色 = Schema 校验错误反馈 + 修正指令

**json_fix_history_mode 配置**：

| 模式 | 行为 | 适用场景 |
|------|------|---------|
| `full`（默认） | 累积所有失败 attempt 的完整对话历史 | 复杂 JSON 修复，需要完整上下文 |
| `last` | 只保留最近一次失败 | 简单修复，节省 token |

`full` 模式下 Attempt 3 能看到 Attempt 1 和 2 的失败，避免重复犯已修好的错误。

**Schema 校验内容**：
- 顶层必填字段：`summary`、`passed`、`issues`
- 类型检查：string / boolean / array
- 每个 issue 的 `severity` 枚举、`line_number` 整数、`message` 非空
- 非法字段名检测（`description`、`fix_suggestion` 等别名）

4. 仍失败则标记 `passed=False`，阻断 commit
