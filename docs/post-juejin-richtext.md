...
正确：组件名使用大驼峰 UserProfile.vue
错误：user-profile.vue（Kebab case 在 Vue 项目中不推荐）

AI 会参考这些案例精准审核代码，相当于把团队规范编程化了。支持按语言分子目录，case_format=compact 时精简注入减少 token。

四级 JSON 容错

AI 返回 JSON 经常出问题，做了四层容错：

L1 本地修复：过滤 think 标签、代码块匹配、括号补全（处理 80% 格式问题）
...
这个工具不是替代人工 CR，而是把低级问题交给 AI，让人去关注更有价值的事。
