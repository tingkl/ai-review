...
一个关键设计：**我们在 system prompt 里明确规定了"📌 注释是代码意图的重要说明"**，AI 必须优先参考注释来判断代码。
...
没有注释时，AI 可能报"不必要的延迟"。但注释明确说明了原因，AI 会尊重注释的判断，不会误报。

**提示词里的具体规则**：
> "代码中的注释是开发者写的意图说明，比代码本身更直接。判断代码是否正确时，要参考注释的说明。如果注释明确解释了某个做法的原因，要尊重注释的判断。不要无视注释，仅凭代码表面就下结论。"

**这意味着**：
- 好注释不仅帮助同事理解代码，还能**直接指导 AI 审核**
- 团队可以**用注释来规定审核边界**——在容易误解的代码旁加注释，降低误报率
- 注释 = 审核规则的一部分，这是工具内置的设计，不是 AI 天生懂注释
