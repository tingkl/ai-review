...
> **📌 注释的重要作用**
>
> 代码中的注释是开发者写的**意图说明**，比代码本身更直接。
>
> **这不是 AI 天生懂注释，而是我们在 system prompt 里明确规定的**：
> > "代码中的注释是开发者写的意图说明，比代码本身更直接。判断代码是否正确时，要参考注释的说明。如果注释明确解释了某个做法的原因，要尊重注释的判断。**当注释与案例出现冲突时，以注释为准。注释的优先级高于案例。**"
>
> 本工具在审核时会**优先参考注释的判断**：
> - ✅ 注释明确解释了某个做法的原因 → 尊重注释，不报错
> - ✅ 注释说明了边界情况的处理逻辑 → 视为合理实现
> - ✅ **注释与案例冲突时 → 以注释为准**（注释优先级高于案例）
> - ❌ 仅凭代码表面就下结论（可能误解意图）
>
> **建议**：在容易引起误解的代码旁添加注释，能显著降低误报率。注释 = 审核规则的一部分，**且优先级高于案例**。
